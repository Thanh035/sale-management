...
export class AlertErrorComponent implements OnDestroy {
  ...
  constructor(private alertService: AlertService, private eventManager: EventManager) {
    this.errorListener = eventManager.subscribe('foodDeliveryBySpringBoot.error', (response: EventWithContent<unknown> | string) => {
      const errorResponse = (response as EventWithContent<AlertError>).content;
      this.addErrorAlert(errorResponse.message);
    });

    this.httpErrorListener = eventManager.subscribe('springBootAngularTsApp.httpError', (response: EventWithContent<unknown> | string) => {
      ...
      switch (httpErrorResponse.status) {
        ...
        case 400: {
          ...
          if (errorHeader) {
            const alertData = entityKey ? { entityName: 'menu' } : undefined;
            ...
            for (const fieldError of fieldErrors) {
              ...
              const fieldName: string = (
                `springBootAngularTsApp.${fieldError.objectName as string}.${convertedField}`
              );
        ...
        case 404:
          this.addErrorAlert('Không tìm thấy');
          break;
